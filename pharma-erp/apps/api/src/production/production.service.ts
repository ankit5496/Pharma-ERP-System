...
export class ProductionService {
  ...
  > {
    ...
    const [items, grouped] = await Promise.all([
      ...
      this.prisma.scoped.stockLot.groupBy({
        by: ['itemId'],
        // The SAME definition the FEFO allocator dispenses against. See
        // issuableStockWhere: these two used to disagree about expired lots,
        // and the gate below was refusing and permitting on different numbers
        // from the ones the store could actually draw.
        where: { itemId: { in: itemIds }, ...issuableStockWhere() },
        _sum: { quantityAvailable: true },
      }),
    ]);
