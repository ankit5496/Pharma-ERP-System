                        ...
                        <ul className="space-y-1">
                          {row.history.map((result) => (
                            <li key={result.id} className="text-[11px] leading-snug">
                              ...
                              {/* Date AND time: two decisions on one batch on one
                                day are a normal sequence — accepted, then held
                                for a retest — and a date alone cannot order
                                them. Who decided sits alongside, because the
                                decision, the person and the moment are one
                                record. */}
                              <span className="block text-slate-500">
                                <DateTimeText value={result.inspectedAt} />
                                {result.inspectedBy && ` · ${result.inspectedBy}`}
                              </span>
                              {result.testReference && (
                                <span className="block text-slate-500">
                                  Ref. {result.testReference}
                                </span>
                              )}
                              {result.remarks && (
