import { UNIT_LABELS, type LowStockItem, type PartySummary } from '@pharma-erp/types';
                  ...
                  <tr key={row.item.id} className="bg-amber-50/30">
                    ...
                    <Td align="right">
                      <span className="font-semibold text-amber-900">
                        <Qty value={row.availableStock} uom={UNIT_LABELS[row.item.uom]} />
                      </span>
                    </Td>

                    <Td align="right">
                      <Qty value={row.item.reorderLevel} uom={UNIT_LABELS[row.item.uom]} />
                    </Td>

                    <Td align="right">
                      <span className="font-semibold text-slate-900">
                        <Qty value={row.shortfall} uom={UNIT_LABELS[row.item.uom]} />
                      </span>
                    </Td>
                    ...
                    <Td align="right">
                      {row.quarantineStock === '0' ? (
                        ...
                        <span title="Received but not yet QC-accepted, so not usable">
                          <Qty value={row.quarantineStock} uom={UNIT_LABELS[row.item.uom]} />
                        </span>
                      )}
                    </Td>
